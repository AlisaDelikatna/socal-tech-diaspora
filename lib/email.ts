...
const from = process.env.EMAIL_FROM ?? "SoCal Tech Diaspora <onboarding@resend.dev>";
...
export function emailLayout(bodyHtml: string): string {
  ...
  <div style="font-family: -apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif; max-width: 560px; margin: 0 auto; padding: 24px; color: #1a1a1a;">
    <div style="border-bottom: 3px solid #2563eb; padding-bottom: 12px; margin-bottom: 20px;">
      <span style="font-size: 18px; font-weight: 700;">SoCal Tech Diaspora</span>
    </div>
    ${bodyHtml}
    <div style="margin-top: 28px; padding-top: 16px; border-top: 1px solid #e5e7eb; font-size: 12px; color: #6b7280;">
      SoCal Tech Diaspora — Ukrainian tech professionals in Southern California.
    </div>
  </div>`;
}
